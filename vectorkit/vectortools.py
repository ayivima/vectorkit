...
class Vector():
	...
	def _errdiff_(self, other, absolute=False):
	...
	def _memsize_(self, stringformat=False):
	...
	def cov(self, other):
		...
		cov_, _, _ = self._covcalcs_(other)
	...
	def corr(self, other):
		...
		cov_, x_std, y_std = self._covcalcs_(other)
	...
	def pad(self, desired_length, extension_component=0):
		...
		self.dimensions = self._dimensions_()
